     ...
     int n;
{
  ...
  while(1){
    ...
    for(i=0;i<20;i++)fprintf(fp,"%g ",d->par[i]);
    fprintf(fp,"%g %g \n",d->norm,d->per);
...
int load_diagram(fp,node)
     ...
     int node;
{
  double u0[NAUTO],uhi[NAUTO],ulo[NAUTO],ubar[NAUTO],evr[NAUTO],evi[NAUTO],norm,par[20],per;
  int i,flag=0;
  int n;
...
/*    start_diagram(NODE); */
  ...
  while(1){
    ...
    for(i=0;i<20;i++)fscanf(fp,"%lg ",&par[i]);
